			...
			<CardContent className="grid gap-3">
				...
				{result ? (
					<div className="rounded-md border bg-muted/40 p-3 text-sm">
						...
						{result.reference ? <div className="mt-1 text-xs text-muted-foreground">Reference: {result.reference}</div> : null}
...
function AcceptanceCaseRunner() {
	...
	const executeCase = async (caseId: string) => {
		...
		setSummary(`${caseId} ${result.data.passed ? "passed" : "failed"} with ${result.data.assertions.length} assertions`);
	};

	const executeAll = async () => {
		const result = await runAll.mutateAsync();
		setLastRun(result.data.results[0] ?? null);
		setSummary(`Provider acceptance suite: ${result.data.passed}/${result.data.total} passed, ${result.data.failed} failed`);
	};
				...
				<div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
					<div>
						...
						<p className="mt-1 text-sm text-muted-foreground">
							Internal MoR/BSP scenarios from the source documents and API collection. Tenants do not operate this suite.
						</p>
					</div>
		...
		<div className="space-y-5">
			...
			<AdminActionPanel
				...
				actions={[
					...
					{ label: "Escalate Shoa credential blocker", action: "tenant.escalate-blocker", targetId: "org_mock_2", variant: "outline" },
				]}
			/>
			<Card>
				<CardContent className="p-0">
		...
		<div className="space-y-5">
			...
			<AdminActionPanel
				...
				actions={[
					...
					{ label: "Mark 7015 manual review", action: "failure.manual-review", targetId: "fail_mock_1", variant: "outline" },
				]}
			/>
			<Card>
				<CardContent className="p-0">
		...
		<div className="space-y-5">
			...
			<AdminActionPanel
				...
				actions={[
					...
					{ label: "Open rotation checklist", action: "certificate.rotation-checklist", targetId: "cert_habesha", variant: "outline" },
				]}
			/>
			<Card>
				<CardContent className="p-0">
