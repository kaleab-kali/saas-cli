			...
			<TableBody>
				{rows.map((row) => (
					<TableRow key={row.join("|")}>
						{row.map((cell, index) => (
							<TableCell key={String(index)} className={index === row.length - 1 ? "max-w-md whitespace-normal" : undefined}>
								{cell}
							</TableCell>
						))}
					</TableRow>
			...
			<CardContent className="grid gap-3">
				{workspace.readiness.steps.map((step) => (
					<div key={step.key} className="rounded-md border p-4">
						...
						{step.actionLabel ? <div className="mt-2 text-xs text-muted-foreground">Next action: {step.actionLabel}</div> : null}
					</div>
				))}
			</CardContent>
		</Card>
...
function SourceReferenceForm({
	...
	const [message, setMessage] = React.useState<string | null>(null);
	React.useEffect(() => setForm((current) => ({ ...current, enterpriseId, establishmentId })), [enterpriseId, establishmentId]);
		...
		<Card>
			...
			<CardContent>
				<p className="mb-4 text-sm text-muted-foreground">
					Enter the register or POS reference for this branch. Your onboarding specialist can help collect it if
		...
		<Card>
			...
			<CardContent className="grid gap-4">
				<p className="text-sm text-muted-foreground">
					Add the API credentials and issued certificate. Most tenants complete this step with an onboarding
					specialist.
				</p>
				<form className="grid gap-4" onSubmit={save}>
					<div className="grid gap-3 md:grid-cols-3">
						...
						<Field
							...
							required
						/>
						<Field label="API key" value={credential.apiKey} onChange={setCredentialValue("apiKey")} autoComplete="off" required />
						<Field
							label="Password"
		...
		<Card>
			...
			<CardContent className="grid gap-4">
				<form className="grid gap-4" onSubmit={submitCancel}>
					<div className="grid gap-3 md:grid-cols-3">
						<Field label="Invoice tax reference" value={cancelForm.invoiceIrn} onChange={setCancelValue("invoiceIrn")} required />
						<SelectField
							label="Reason code"
...
export function EimsOverviewPage() {
	...
	if (overviewQuery.isLoading || workspaceQuery.isLoading || !overviewQuery.data || !workspaceQuery.data) return <LoadingPanel />;
				...
				<Card>
					...
					<CardContent className="p-0">
						<DataTable
							...
							rows={setup.enterprises.map((row) => [row.legalName, row.tin, row.vatNumber, businessStatusLabel(row.status)])}
						/>
					</CardContent>
				</Card>
			</div>
				...
				<Card>
					...
					<CardContent className="p-0">
						<DataTable
							...
							rows={setup.establishments.map((row) => [row.name, row.code, row.subTin, row.city, businessStatusLabel(row.status)])}
						/>
					</CardContent>
				</Card>
			</div>
	...
	return (
		<div className="space-y-5">
			<PageHeader
				...
				description="Certificate validity for each approved register/POS."
			/>
			<Card>
				<CardContent className="p-0">
					<DataTable
...
export function EimsCompliancePage() {
	...
	if (evidence.isLoading || !evidence.data) return <LoadingPanel />;
