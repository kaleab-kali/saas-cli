...
function buildAssertions(caseId: EimsAcceptanceCaseId, request: unknown, response: unknown): AssertionResult[] {
	...
	const invoiceRequest = req.request as Record<string, any> | undefined;

	switch (caseId) {
		case "IRC-P01":
			return [
				assert("B2C transaction type", invoiceRequest?.TransactionType, "B2C"),
				assert("Buyer TIN is null", String(invoiceRequest?.BuyerDetails?.Tin), "null"),
				assertContains("VAT0 item exists", invoiceRequest?.ItemList?.map((item: any) => item.TaxCode), "VAT0"),
				assertContains("VAT15 item exists", invoiceRequest?.ItemList?.map((item: any) => item.TaxCode), "VAT15"),
				assertContains("VATEX item exists", invoiceRequest?.ItemList?.map((item: any) => item.TaxCode), "VATEX"),
				assertExists("EIMS IRN returned", res.Irn),
				assertExists("EIMS signed QR returned", res.SignedQR),
			];
		case "IRC-P02":
			return [
				assert("B2B transaction type", invoiceRequest?.TransactionType, "B2B"),
				assertRegex("Buyer TIN is 10 digits", invoiceRequest?.BuyerDetails?.Tin, /^\d{10}$/),
				assertExists("Buyer legal name exists", invoiceRequest?.BuyerDetails?.LegalName),
				assertExists("Withholding value exists", invoiceRequest?.ValueDetails?.TransactionWithholdValue),
				assertExists("EIMS IRN returned", res.Irn),
			];
		case "IRC-P03":
			return [
				assert("Receipt references accepted invoice", req.Invoices?.[0]?.InvoiceIRN, acceptedInvoiceIrn),
				assert("Payment coverage is full", req.Invoices?.[0]?.PaymentCoverage, "full"),
				assertExists("RRN returned", res.Rrn),
				assertExists("Signed receipt QR returned", res.SignedQR),
			];
		case "IRC-P04":
			return [
				assert("Withholding type is TWHT", req.WithholdDetail?.Type, "TWHT"),
				assertNumberGreaterThan("Withholding amount greater than zero", req.WithholdDetail?.WithholdingAmount, 0),
				assertExists("Withholding receipt RRN returned", res.Rrn),
			];
		case "IRC-P05":
			return [
				assert("Cancellation IRN matches accepted invoice", req.Irn, acceptedInvoiceIrn),
				assert("Reason code is Others", req.ReasonCode, "4"),
				assertExists("Remark supplied for reason 4", req.Remark),
				assert("Cancellation accepted", res.Status, "cancelled"),
			];
		case "IRC-P06":
			return [
				assert("Document type is credit note", invoiceRequest?.DocumentDetails?.Type, "CRE"),
				assertExists("Credit note reason exists", invoiceRequest?.DocumentDetails?.Reason),
				assert("Related document references original invoice", invoiceRequest?.DocumentDetails?.RelatedDocument, acceptedInvoiceIrn),
				assertExists("Credit note IRN returned", res.Irn),
			];
		case "IRC-P07":
			return [
				assert("Document type is debit note", invoiceRequest?.DocumentDetails?.Type, "DEB"),
				assertExists("Debit note reason exists", invoiceRequest?.DocumentDetails?.Reason),
				assert("Related document references original B2B invoice", invoiceRequest?.DocumentDetails?.RelatedDocument, acceptedB2bIrn),
				assertExists("Debit note IRN returned", res.Irn),
			];
		case "IRC-N08":
			return [
				assert("Invalid request is B2B", invoiceRequest?.TransactionType, "B2B"),
				assert("Invalid buyer TIN remains invalid", invoiceRequest?.BuyerDetails?.Tin, "123"),
				assertExists("Validation error returned", res.ErrorCode),
				assert("No IRN is issued", String(res.Irn), "undefined"),
			];
		case "IRC-N09":
			return [
				assert("Receipt references inactive IRN", req.Invoices?.[0]?.InvoiceIRN, "MOCK-IRN-CANCELLED-OR-MISSING"),
				assertExists("Receipt rejection returned", res.ErrorCode),
				assert("No RRN is issued", String(res.Rrn), "undefined"),
			];
		case "IRC-N010":
			return [
				assert("Cancellation references inactive IRN", req.Irn, "MOCK-IRN-ALREADY-CANCELLED"),
				assertExists("Cancellation rejection returned", res.ErrorCode),
				assert("No cancellation status returned", String(res.Status), "undefined"),
			];
		case "ADD-N001":
			return [
				assertContains("SMS channel exists", res.Notifications?.map((item: any) => item.channel), "sms"),
				assertContains("Email channel exists", res.Notifications?.map((item: any) => item.channel), "email"),
				assertContains("SMS provider configured", res.Notifications?.map((item: any) => item.provider), "Africa's Talking"),
			];
		case "ADD-C001":
			return [
				assertRegex("Enterprise TIN is 10 digits", res.Enterprise?.Tin, /^\d{10}$/),
				assertRegex("Sub-TIN format is valid", res.Establishment?.SubTin, /^\d{10}-\d{2}$/),
				assertExists("MoR source number exists", res.SourceSystem?.SystemNumber),
				assert("Credential test is successful", res.Credential?.lastTestStatus, "success"),
				assert("Secrets are not returned", String(res.Credential?.secretsReturned), "false"),
			];
		case "ADD-P001":
			return [
				assertContains("Compact print layout exists", res.PrintLayouts?.map((item: any) => item.layout), "compact"),
				assertContains("A4 print layout exists", res.PrintLayouts?.map((item: any) => item.layout), "a4"),
				assertContains("IRN is mandatory", res.MandatoryFields, "IRN"),
				assertContains("QR is mandatory", res.MandatoryFields, "QR"),
			];
	}
}

function buildEvidence(definition: EimsAcceptanceCaseDefinition, fixture: { request: unknown; response: unknown }) {
	const response = fixture.response as Record<string, any>;
...
function assertExists(name: string, actual: unknown): AssertionResult {
	return { name, passed: actual !== undefined && actual !== null && actual !== "", expected: "present", actual: String(actual) };
}
