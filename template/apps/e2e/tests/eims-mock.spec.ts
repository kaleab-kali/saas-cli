...
test.describe("tenant EIMS workflow is business-facing and backend-driven", () => {
	test("tenant navigation exposes business actions, not internal compliance/source modules", async ({ page }) => {
		...
		for (const hidden of ["Enterprises", "Branches", "MoR Sources", "Credentials", "Certificates", "Compliance", "Bulk"]) {
	...
	test("status page renders required tenant inputs, setup state, and invoice data", async ({
		page,
	}) => {
		...
		for (const invoice of overview.recentSubmissions) {
			await expectRowContains(page, invoice.documentNumber, [
				...
				invoice.irn ?? "Pending EIMS acceptance",
			]);
			if (invoice.status === "pending_offline") {
				expect(invoice.irn).toBeNull();
				await expectRowContains(page, invoice.documentNumber, ["Pending EIMS acceptance"]);
			}
		}
		await assertNoTenantInternalLanguage(page);
	});
	...
	test("guided setup submits tenant-provided business, branch, source, credential, and certificate data", async ({
		...
		await expect(page.getByRole("heading", { name: "Guided EIMS setup" })).toBeVisible();
		...
		await page.getByLabel("MoR-approved system number").fill("329D03B6F0");
		...
		await page.getByRole("button", { name: "Save register/POS reference" }).click();
		...
		await page.getByLabel("MoR client ID").fill("client-front-pos");
		await page.getByLabel("MoR username").fill("TIN0074136947");
		...
		await page.getByRole("button", { name: "Generate CSR for INSA" }).click();
		...
		await page.getByLabel("Issued certificate PEM").fill("-----BEGIN CERTIFICATE-----\\nTEST\\n-----END CERTIFICATE-----");
		...
		const certImport = waitForJson<ApiEnvelope<{ message: string }>>(
			page,
			"/api/v1/eims/certificates/import",
			201,
		);
	...
	test("invoice, receipt, cancellation, batch, and export pages validate live backend payloads", async ({ page }) => {
		...
		await page.getByRole("button", { name: "Submit sample invoice" }).click();
		expect(await submitRequest).toMatchObject({ documentNumber: expect.stringMatching(/^INV-TEST-/) });
		...
		const cancelSubmit = waitForJson<ApiEnvelope<{ message: string }>>(
			page,
			"/api/v1/eims/cancellations",
			202,
		);
...
test.describe("super-admin EIMS operations owns provider compliance and BSP evidence", () => {
	test("admin pages render tenant readiness, failures, resources, certificates, and evidence data", async ({ page }) => {
	...
	test("admin acceptance runner executes every provider case with data assertions", async ({ page }) => {
		const casesPromise = waitForJson<ApiEnvelope<Array<{ caseId: string; endpoint: string; requiredEvidence: string[] }>>>(
			page,
			"/api/v1/admin/eims/acceptance/cases",
		);
