...
Production readiness is enforced by `pnpm doctor:production`. For an EIMS install it blocks launch when the app is still in mock mode, lacks production MoR/EIMS URLs, lacks an HTTPS callback URL, lacks a callback HMAC secret, uses local signing, or has Phase 0 strict mode disabled.
