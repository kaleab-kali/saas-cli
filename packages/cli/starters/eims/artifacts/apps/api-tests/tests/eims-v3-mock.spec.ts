...
test.describe("EIMS V3 backend mock API contract", () => {
	...
	test("tenant setup endpoint exposes saved setup state for guided forms", async ({ request }) => {
		const response = await request.get("/api/v1/eims/setup");
		expect(response.status()).toBe(200);
		const body = await response.json();
		...
		expect(body.data.enterprises[0]).toMatchObject({
			tin: "0074136947",
			legalName: "Habesha Restaurant PLC",
		});
		expect(body.data.establishments[0]).toMatchObject({
			code: "BOL",
			subTin: "0074136947-01",
		});
		expect(body.data.sourceSystems.map((source: { approvalStatus: string }) => source.approvalStatus)).toEqual(
			expect.arrayContaining(["approved", "pending_mor_approval"]),
		);
	});
