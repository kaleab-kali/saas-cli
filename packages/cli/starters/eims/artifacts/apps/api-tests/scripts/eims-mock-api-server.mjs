...
const eimsOnboardingSteps = [
	[
		"tenant-intake",
		"Tenant intake",
		"Capture legal name, TIN, owner contact, and payment proof.",
		"setup",
		"STAFF",
		"COMPLETED",
	],
	[
		...
		"Subscription verified",
		"Confirm the EIMS add-on and receipt before authority work starts.",
		"billing",
		"STAFF",
		"COMPLETED",
	],
	[
		"organization-shell",
		"Organization shell",
		"Create the tenant, owner user, settings, and VAT metadata.",
		"setup",
		"STAFF",
		"COMPLETED",
	],
	[
		"mor-portal-signup",
		"MoR portal signup",
		"Submit the portal request and coordinate the OTP handoff.",
		"authority",
		"STAFF",
		"IN_PROGRESS",
	],
	[
		"portal-login-2fa",
		"Portal login and 2FA",
		"Change password and store backup codes through the secure workflow.",
		"authority",
		"STAFF",
		"PENDING",
	],
	[
		...
		"Register the POS source and capture branch metadata.",
		"authority",
		"STAFF",
		"PENDING",
	],
	[
		"mor-approval-wait",
		"MoR approval wait",
		"Track source approval status and follow-up notes.",
		"authority",
		"STAFF",
		"PENDING",
	],
	[
		"credential-capture",
		"Capture API credentials",
		"Store client ID, client secret, API key, username, and system number.",
		"credential",
		"STAFF",
		"PENDING",
	],
	[
		"generate-csr",
		"Generate CSR",
		"Create the CSR and encrypted private key material.",
		"certificate",
		"STAFF",
		"PENDING",
	],
	[
		"send-insa-request",
		"Send INSA request",
		"Send the CSR email package and request form.",
		"certificate",
		"STAFF",
		"PENDING",
	],
	[
		"certificate-upload",
		"Upload certificate",
		"Import the issued certificate and validate key match, TIN, and expiry.",
		"certificate",
		"STAFF",
		"PENDING",
	],
	[
		"controlled-test-invoice",
		"Controlled test invoice",
		"Capture controlled test IRN and signed QR evidence before go-live.",
		"verification",
		"STAFF",
		"PENDING",
	],
	[
		"tenant-notified-live",
		"Notify tenant",
		"Send SMS, email, and training appointment details.",
		"launch",
		"STAFF",
		"PENDING",
	],
	[
		"first-live-invoice",
		"First live invoice",
		"Observe the first production IRN with the cashier.",
		"launch",
		"TENANT",
		"PENDING",
	],
	[
		"evidence-archive",
		"Evidence archive",
		"Archive MoR, INSA, invoice, training, and renewal evidence.",
		"launch",
		"STAFF",
		"PENDING",
	],
	...
	activities: [
		{
			id: "act_eims_1",
			type: "STAFF_ACTION",
			message: "EIMS concierge onboarding started",
			userId: "admin_1",
			createdAt: fixedNow,
		},
		{
			id: "act_eims_2",
			type: "SYSTEM",
			message: "Steps 1-3 auto-completed after tenant creation",
			userId: null,
			createdAt: fixedNow,
		},
	],
...
const tenantWorkspace = {
	data: {
		...
		supportNote:
			"If the tenant does not have these yet, onboarding staff can guide them through the MoR portal and INSA certificate request.",
		primaryActions: [
		...
		alerts: [
			...
			{
				level: "info",
				message: "Invoices waiting for sync do not show an official IRN or QR until EIMS accepts them.",
			},
		],
	},
};
...
const setupState = () => ({
	data: {
		status: overview.data.mode,
		counts: {
			enterprises: overview.data.enterprises.length,
			establishments: overview.data.establishments.length,
			sourceSystems: overview.data.sourceSystems.length,
		},
		enterprises: overview.data.enterprises,
		establishments: overview.data.establishments,
		sourceSystems: overview.data.sourceSystems,
	},
});
...
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: mock route dispatch intentionally mirrors tenant EIMS endpoints.
async function handleTenantEimsRoutes(req, res, path) {
	...
	if (path === "/api/v1/eims/setup" && req.method === "GET") return sendJson(res, 200, setupState());
	...
	if (
		(path === "/api/v1/eims/bulk/reconcile" || path === "/api/v1/eims/bulk/mock-reconcile") &&
		req.method === "POST"
	) {
		const body = await readJson(req);
		return sendJson(res, 202, {
	...
	if (path === "/api/v1/eims/compliance/evidence" && req.method === "GET")
		return sendJson(res, 200, complianceEvidence());
	if (
	...
	switch (caseId) {
		case "IRC-P01":
			return invoice({
				...
				taxCodes: ["VAT15", "VAT0", "VATEX"],
				irn: eimsSubmissions[0].irn,
			});
		case "IRC-P02":
			return invoice({
				transactionType: "B2B",
		...
		case "IRC-P05":
			return {
				request: { Irn: eimsSubmissions[0].irn, ReasonCode: "4", Remark: "Customer returned the order" },
				response: { StatusCode: 200, Status: "cancelled" },
			};
		case "IRC-P06":
			return invoice({
				documentType: "CRE",
				documentNumber: "CRE-2026-00003",
				relatedDocument: eimsSubmissions[0].irn,
		...
		case "IRC-N09":
			return {
				request: { Invoices: [{ InvoiceIRN: "MOCK-IRN-CANCELLED-OR-MISSING" }] },
				response: { StatusCode: 406, ErrorCode: "7019", Message: "Invoice IRN is not active" },
			};
		case "IRC-N010":
			return {
				request: { Irn: "MOCK-IRN-ALREADY-CANCELLED", ReasonCode: "1" },
				response: { StatusCode: 406, ErrorCode: "7002", Message: "Invoice already cancelled" },
			};
		case "ADD-N001":
		...
		case "ADD-C001":
			return {
				...
				response: {
					Enterprise: overview.data.enterprises[0],
					Establishment: overview.data.establishments[0],
					SourceSystem: overview.data.sourceSystems[0],
					Credential: credentials.data[0],
				},
			};
		case "ADD-P001":
			return {
				request: { layouts: ["compact", "a4"] },
				response: {
					PrintLayouts: printLayouts.data,
					MandatoryFields: ["IRN", "QR", "seller TIN", "tax value", "total value"],
				},
			};
		default:
...
function acceptanceAssertions(caseId, request, response) {
	...
	const assertionsByCase = {
		"IRC-P01": [
			...
			assertion(
				"VAT0 item exists",
				req.ItemList?.some((item) => item.TaxCode === "VAT0"),
				"VAT0",
				req.ItemList?.map((item) => item.TaxCode).join(","),
			),
			assertion(
				"VAT15 item exists",
				req.ItemList?.some((item) => item.TaxCode === "VAT15"),
				"VAT15",
				req.ItemList?.map((item) => item.TaxCode).join(","),
			),
			assertion(
				"VATEX item exists",
				req.ItemList?.some((item) => item.TaxCode === "VATEX"),
				"VATEX",
				req.ItemList?.map((item) => item.TaxCode).join(","),
			),
		...
		"IRC-P02": [
			...
			assertion(
				"Buyer TIN is 10 digits",
				/^\d{10}$/.test(req.BuyerDetails?.Tin ?? ""),
				"10 digits",
				req.BuyerDetails?.Tin,
			),
			assertion(
				"Withholding value exists",
				req.ValueDetails?.TransactionWithholdValue === "600.00",
				"600.00",
				req.ValueDetails?.TransactionWithholdValue,
			),
			assertion("IRN returned", Boolean(response.Irn), "present", response.Irn),
		],
		"IRC-P03": [
			assertion(
				"Receipt references accepted invoice",
				req.Invoices?.[0]?.InvoiceIRN === eimsSubmissions[0].irn,
				eimsSubmissions[0].irn,
				req.Invoices?.[0]?.InvoiceIRN,
			),
		...
		"IRC-P04": [
			...
			assertion(
				"Withholding amount greater than zero",
				Number(req.WithholdDetail?.WithholdingAmount) > 0,
				"> 0",
				req.WithholdDetail?.WithholdingAmount,
			),
			assertion("RRN returned", Boolean(response.Rrn), "present", response.Rrn),
		],
		"IRC-P05": [
		...
		"IRC-P06": [
			...
			assertion(
				...
				Boolean(req.DocumentDetails?.RelatedDocument),
				"present",
				req.DocumentDetails?.RelatedDocument,
			),
			assertion("IRN returned", Boolean(response.Irn), "present", response.Irn),
		],
		"IRC-P07": [
		...
		"IRC-N09": [
			assertion(
				"Inactive IRN used",
				req.Invoices?.[0]?.InvoiceIRN === "MOCK-IRN-CANCELLED-OR-MISSING",
				"inactive",
				req.Invoices?.[0]?.InvoiceIRN,
			),
			assertion("Error returned", Boolean(response.ErrorCode), "present", response.ErrorCode),
		...
		"ADD-N001": [
			assertion(
				...
				response.Notifications?.some((item) => item.channel === "sms"),
				"sms",
				response.Notifications?.map((item) => item.channel).join(","),
			),
			assertion(
				"Email notification exists",
				response.Notifications?.some((item) => item.channel === "email"),
				"email",
				response.Notifications?.map((item) => item.channel).join(","),
			),
		],
		"ADD-C001": [
			assertion(
				"TIN is 10 digits",
				/^\d{10}$/.test(response.Enterprise?.tin ?? response.Enterprise?.Tin ?? ""),
				"10 digits",
				response.Enterprise?.tin ?? response.Enterprise?.Tin,
			),
			assertion(
				"Sub-TIN is valid",
				/^\d{10}-\d{2}$/.test(response.Establishment?.subTin ?? ""),
				"TIN-NN",
				response.Establishment?.subTin,
			),
			assertion(
				"Credential tested",
				response.Credential?.lastTestStatus === "success",
				"success",
				response.Credential?.lastTestStatus,
			),
		],
		"ADD-P001": [
			assertion(
				"Compact layout exists",
				response.PrintLayouts?.some((item) => item.layout === "compact"),
				"compact",
				response.PrintLayouts?.map((item) => item.layout).join(","),
			),
			assertion(
				"A4 layout exists",
				response.PrintLayouts?.some((item) => item.layout === "a4"),
				"a4",
				response.PrintLayouts?.map((item) => item.layout).join(","),
			),
