...
const EIMS_SDK_BULK_SUBMITTERS = [
	"submitBulk",
	"submitBulkInvoices",
	"registerBulkInvoices",
	"submitBulkDocuments",
] as const;
...
export const missingEimsSdkCapabilities = (value: unknown) => {
	if (!value || typeof value !== "object") return [...REQUIRED_EIMS_SDK_CAPABILITIES, "validateCredential"];
	const candidate = value as Record<string, unknown>;
	const missing: string[] = REQUIRED_EIMS_SDK_CAPABILITIES.filter(
	...
	if (!EIMS_SDK_BULK_SUBMITTERS.some((capability) => typeof candidate[capability] === "function")) {
		missing.push("submitBulk");
	}
...
export const createEimsSdkClientFromModule = async (
	...
	throw new ServiceUnavailableException(
		"EIMS SDK package does not expose a registerInvoice/registerReceipt/verifyIrn/validateCredential/submitBulk/pollBulkStatus/cancelInvoice-capable client",
	);
};
