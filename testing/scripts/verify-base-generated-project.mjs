...
function assertDeployGateBuilds() {
	...
	assert(preLaunchChecklist.includes("METRICS_TOKEN"), "pre-launch checklist includes metrics protection");
	assert(preLaunchChecklist.includes("SMTP_HOST"), "pre-launch checklist includes verified email relay");
	assert(rootEnvExample.includes("METRICS_TOKEN="), "root env reference includes metrics token");
	assert(rootEnvExample.includes("SMTP_HOST="), "root env reference includes SMTP host");
	assert(productionEnvExample.includes("METRICS_TOKEN="), "production env example includes metrics token");
	assert(productionEnvExample.includes("SMTP_FROM="), "production env example includes SMTP sender");
	assert(apiEnvExample.includes("API_RATE_LIMIT_PER_TENANT="), "API env example includes tenant rate-limit config");
